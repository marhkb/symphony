...
mod imp {
    ...
    impl NetworksPanel {
        ...
        pub(super) fn set_network_list(&self, value: Option<&model::NetworkList>) {
            let obj = &*self.obj();
            if obj.network_list().as_ref() == value {
                return;
            }
            ...
            if let Some(container_list) = value {
                ...
                self.flow_box.bind_model(Some(&model), |item| {
                    gtk::FlowBoxChild::builder()
                        ...
                        // .child(&gtk::Label::builder().label("label").build())
                        .build()
                        .upcast()
                });
    ...
    pub(crate) fn delete_selection(&self) {
        ...
        dialog.connect_response(
            ...
            clone!(@weak self as obj => move |_, response| if response == "delete" {
                if let Some(list) = obj.network_list() {
                    list
                        .selected_items()
                        .iter()
                        .map(|obj| obj.downcast_ref::<model::Container>().unwrap())
